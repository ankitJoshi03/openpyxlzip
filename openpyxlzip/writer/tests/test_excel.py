...
from openpyxlzip import load_workbook
...
def test_write_chart(ExcelWriter, archive):
    ...
    rel = ws._rels["rId1"]
    assert dict(rel) == {'Id': 'rId1', 'Target': '../drawings/drawing1.xml',
                         'Type':
...
def test_comment(ExcelWriter, archive):
    ...
    assert archive.namelist() == ['xl/comments/comment1.xml', 'xl/drawings/vmlDrawing1.vml']
    assert '/xl/comments/comment1.xml' in writer.manifest.filenames
    assert ws.legacy_drawing == 'xl/drawings/vmlDrawing1.vml'
...
def test_duplicate_chart(ExcelWriter, archive):
    from openpyxlzip.chart import PieChart
    pc = PieChart()
    wb = Workbook()
    writer = ExcelWriter(wb, archive)
