...
class Sequence(Descriptor):
    ...
    def to_tree(self, tagname, obj, namespace=None, elem_type=None):
        """
        ...
        for idx, v in enumerate(obj, self.idx_base):
            if hasattr(v, "to_tree"):
                el = v.to_tree(tagname, idx, elem_type=elem_type)
            else:
...
class NestedSequence(Sequence):
    ...
    def to_tree(self, tagname, obj, namespace=None, elem_type=None):
        ...
        for v in obj:
            container.append(v.to_tree(elem_type=elem_type))
        return container
...
class MultiSequence(Sequence):
    ...
    def to_tree(self, tagname, obj, namespace=None, elem_type=None):
        ...
        """
        for v in obj:
            el = v.to_tree(namespace=namespace, elem_type=elem_type)
            yield el
