...
from openpyxlzip.utils import absolute_coordinate, quote_sheetname
...
from openpyxlzip.workbook.defined_name import DefinedName
from openpyxlzip.workbook.external_reference import ExternalReference
from openpyxlzip.packaging.workbook import ChildSheet, WorkbookPackage, PivotCache
from openpyxlzip.workbook.properties import WorkbookProperties
from openpyxlzip.utils.datetime import CALENDAR_MAC_1904
...
class WorkbookWriter:
    ...
    def __init__(self, wb):
        ...
        #Create the workbook package and fill in extra properties
        self.package = WorkbookPackage()
        self.package.workbookProtection = wb.security
        self.package.calcPr = wb.calculation
        #TODO add more MattJ
        if wb.package is not None:
            if hasattr(wb.package, "nsmaps"):
                self.package.nsmaps = wb.package.nsmaps
            if hasattr(wb.package, "elem_order"):
                self.package.elem_order = wb.package.elem_order
            self.package.fileVersion = wb.package.fileVersion
            if hasattr(wb.package, "extra_attr"):
                self.package.extra_attr = wb.package.extra_attr
            if hasattr(wb.package, "extra_elem"):
                self.package.extra_elem = wb.package.extra_elem
    ...
    def write_properties(self):
        ...
        if hasattr(self.wb, "defaultThemeVersion") and self.wb.defaultThemeVersion is not None:
            props.defaultThemeVersion = self.wb.defaultThemeVersion
    ...
    def write_worksheets(self):
        for idx, sheet in enumerate(self.wb._sheets, 1):
            if sheet.sheetId is None:
                sheet.sheetId = idx
            sheet_node = ChildSheet(name=sheet.title, sheetId=sheet.sheetId, id="rId{0}".format(idx))
            rel = Relationship(type=sheet._rel_type, Target=sheet.path.replace("/xl/", ""))
    ...
    def write_refs(self):
        for link in self.wb._external_links:
            ...
            rel = Relationship(type=link._rel_type, Target=link.path.replace("/xl/", ""))
            self.rels.append(rel)
    ...
    def write_names(self):
        ...
        # print("Saved defined names", self.package.definedNames)
    ...
    def write(self):
        ...
        if LXML:
            # print("Should be outputting", self.package.definedNames, tostring(self.package.definedNames.to_tree(verbose=True)))
            return tostring(self.package.to_tree(), pretty_print = True, xml_declaration = True, encoding='UTF-8', standalone=True)
        else:
    ...
    def write_rels(self):
        ...
        if self.wb._custom_xml is not None:
            for pathname in self.wb._custom_xml:
                if "_rels" not in pathname and "Props" not in pathname:
                    custom_xml =  Relationship(type='customXml', Target="../" + pathname)
                    self.rels.append(custom_xml)
        ...
        if LXML:
            return tostring(self.rels.to_tree(), pretty_print = True, xml_declaration = True, encoding='UTF-8', standalone=True)
        else:
    ...
    def write_root_rels(self):
        ...
        #MattJ added the custom xml
        if self.wb.arc_custom is not None:
        ...
        if LXML:
            return tostring(rels.to_tree(), pretty_print = True, xml_declaration = True, encoding='UTF-8', standalone=True)
        else:
