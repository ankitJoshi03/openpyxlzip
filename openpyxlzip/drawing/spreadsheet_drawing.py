...
from openpyxlzip.utils import coordinate_to_tuple
from openpyxlzip.utils.units import (
    cm_to_EMU,
    pixels_to_EMU,
)
from openpyxlzip.drawing.image import Image

from openpyxlzip.xml.constants import SHEET_DRAWING_NS, DRAWING_NS, MC_NS, DRAWING_14_NS
from openpyxlzip.xml.functions import Element

from openpyxlzip.chart._chart import ChartBase
from .xdr import (
    XDRPoint2D,
    XDRPositiveSize2D,
)
...
class SpreadsheetDrawing(Serialisable):
    ...
    def __init__(self,
        ...
        self.images = []
        self._rels = []
        self.wrap_alternate_content = wrap_alternate_content
    ...
    def _write(self):
        ...
        anchors = []
        self.elem_order = {
                            0: "{%s}twoCellAnchor" % SHEET_DRAWING_NS,
                            1: "{%s}oneCellAnchor" % SHEET_DRAWING_NS,
                            2: "{%s}absoluteAnchor" % SHEET_DRAWING_NS,
                        }
        self._rels = []
        for idx, obj in enumerate(self.charts + self.images, 1):
            ...
            if isinstance(obj, ChartBase):
                ...
                rel = Relationship(Id="rId{0}".format(idx), type="image", Target=obj.path.replace("/xl/", "../").replace(".wmf", ".emf"))
        ...
        for a in anchors:
            if isinstance(a, OneCellAnchor):
                found = False
                idx = 0
                for other_a in self.oneCellAnchor:
                    if other_a._from.col == a._from.col and other_a._from.row == a._from.row:
                        found = True
                        break
                    idx += 1
                if not found:
                    ...
                    self.oneCellAnchor[idx] = a
            elif isinstance(a, TwoCellAnchor):
                found = False
                idx = 0
                for other_a in self.twoCellAnchor:
                    if other_a._from.col == a._from.col and other_a._from.row == a._from.row:
                        found = True
                        break
                    idx += 1
                if not found:
                    ...
                    self.twoCellAnchor[idx] = a
            else:
                self.absoluteAnchor.append(a)
        self.nsmaps = {"xdr": SHEET_DRAWING_NS, "a": DRAWING_NS}
        tree = self.to_tree()
        #Manually fix the namespaces
        for child_node in tree.iterdescendants():
            if "{" not in child_node.tag:
                old_tag = child_node.tag
                child_node.tag = "{" + SHEET_DRAWING_NS + "}" + child_node.tag
        #Manually wrap in altcontent
        print("Writing")
        if self.wrap_alternate_content:
            print("Wrapping")
            root_node = self.to_tree()
            for child in list(root_node):
                root_node.remove(child)
            for child_node in tree.iterchildren():
                tagname = "{%s}%s" % (MC_NS, "AlternateContent")
                alt_content = Element(tagname, {}, nsmap={"mc": MC_NS})

                tagname = "{%s}%s" % (MC_NS, "Choice")
                choice = Element(tagname, {"Requires": "a14"}, nsmap={"a14": DRAWING_14_NS})
                choice.append(child_node)
