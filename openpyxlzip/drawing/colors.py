...
from openpyxlzip.descriptors.excel import Percentage
...
from openpyxlzip.styles.colors import RGB
from openpyxlzip.xml.constants import DRAWING_NS, DRAWING_14_NS, MC_NS
...
class SRGBClr(Serialisable):

    tagname = "colorChoice"
    namespace = DRAWING_NS

    val = String(allow_none=True)
    Ignorable = String(namespace=MC_NS, allow_none=True)
    legacySpreadsheetColorIndex = String(namespace=DRAWING_14_NS, allow_none=True)

    def __init__(self,
                 val=None,
                 legacySpreadsheetColorIndex=None,
                 Ignorable=None
                ):
        self.val = val
        self.legacySpreadsheetColorIndex = legacySpreadsheetColorIndex
...
class ColorChoice(Serialisable):
    ...
    srgbClr = Typed(expected_type=SRGBClr, allow_none=True) #NestedValue(expected_type=str, allow_none=True) # needs pattern and can have transform
    RGB = Alias('srgbClr')
...
from openpyxlzip.descriptors.office_art import OfficeArtExtensionList
