...
from openpyxlzip.chart import BarChart
...
class TestSpreadsheetDrawing:
    ...
    def test_ctor(self, SpreadsheetDrawing):
        ...
        """
        xml = xml.decode().replace("{http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing}", "").replace("xdr:", "").replace(' xmlns:xdr="http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing"', "")
        diff = compare_xml(xml, expected)
        assert diff is None, diff
    ...
    def test_write_rels(self, SpreadsheetDrawing):
        from openpyxlzip.packaging.relationship import Relationship
