...
from openpyxlzip.descriptors.sequence import NestedSequence
from openpyxlzip.descriptors.excel import ExtensionList
from openpyxlzip.utils.indexed_list import IndexedList
from openpyxlzip.xml.constants import ARC_STYLE, SHEET_MAIN_NS
...
class Stylesheet(Serialisable):
    ...
    def to_tree(self, tagname=None, idx=None, namespace=None, elem_type=None):
        root_tag = "{http://schemas.openxmlformats.org/spreadsheetml/2006/main}styleSheet"
        from openpyxlzip.xml.schema import ROOT_ELEMS
        root_type = ROOT_ELEMS[root_tag]
        tree = super(Stylesheet, self).to_tree(tagname, idx, namespace, elem_type=root_type)
...
def apply_stylesheet(archive, wb):
    ...
    wb._stylesheet_colors = stylesheet.colors
    wb._stylesheet_extLst = stylesheet.extLst
    if hasattr(stylesheet, "nsmaps"):
        wb._stylesheet_nsmaps = stylesheet.nsmaps
    if hasattr(stylesheet, "elem_order"):
        wb._stylesheet_elem_order = stylesheet.elem_order
    if hasattr(stylesheet, "extra_attr"):
        wb._stylesheet_extra_attr = stylesheet.extra_attr
    if hasattr(stylesheet, "extra_elem"):
        wb._stylesheet_extra_elem = stylesheet.extra_elem


    # need to overwrite openpyxlzip defaults in case workbook has different ones
    ...
    if not wb._named_styles:
        ...
        warn("Workbook contains no default style, apply openpyxlzip's default")
...
def write_stylesheet(wb):
    ...
    if hasattr(wb, "_stylesheet_colors"):
        stylesheet.colors = wb._stylesheet_colors
    if hasattr(wb, "_stylesheet_nsmaps"):
        stylesheet.nsmaps = wb._stylesheet_nsmaps
    if hasattr(wb, "_stylesheet_extLst"):
        stylesheet.extLst = wb._stylesheet_extLst
    if hasattr(wb, "_stylesheet_extra_attr"):
        stylesheet.extra_attr = wb._stylesheet_extra_attr
    if hasattr(wb, "_stylesheet_extra_elem"):
        stylesheet.extra_elem = wb._stylesheet_extra_elem
    if hasattr(wb, "_stylesheet_elem_order"):
        stylesheet.elem_order = wb._stylesheet_elem_order
    #TODO
