...
from openpyxlzip.descriptors import String, Sequence
...
mimetypes.add_type("application/vnd.openxmlformats-officedocument.spreadsheetml.printerSettings", ".bin")
...
class Manifest(Serialisable):
    ...
    def append_manual(self, path, mime_type):
        """
        Add content object to the package manifest
        # needs a contract...
        """
        ct = Override(PartName=path, ContentType=mime_type)
        self.Override.append(ct)
    ...
    def _write(self, archive, workbook):
        ...
        if LXML:
            archive.writestr(self.path, tostring(self.to_tree(), pretty_print = True, xml_declaration = True, encoding='UTF-8', standalone=True))
        else:
    ...
    def _write_vba(self, workbook):
        ...
        if workbook.vba_archive:
            ...
            for override in mf.Override:
                if override.ContentType not in (ACTIVEX, ACTIVEX_BIN, CTRL, VBA, DRAWING_TYPE):
                    continue
