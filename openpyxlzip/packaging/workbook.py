...
from openpyxlzip.descriptors.excel import ExtensionList, Relation
from openpyxlzip.descriptors.sequence import NestedSequence
from openpyxlzip.descriptors.nested import NestedString

from openpyxlzip.xml.constants import SHEET_MAIN_NS
from openpyxlzip.xml.schema import ROOT_ELEMS

from openpyxlzip.workbook.defined_name import DefinedName, DefinedNameList
from openpyxlzip.workbook.external_reference import ExternalReference
from openpyxlzip.workbook.function_group import FunctionGroupList
from openpyxlzip.workbook.properties import WorkbookProperties, CalcProperties, FileVersion
from openpyxlzip.workbook.protection import WorkbookProtection, FileSharing
from openpyxlzip.workbook.smart_tags import SmartTagList, SmartTagProperties
from openpyxlzip.workbook.views import CustomWorkbookView, BookView
from openpyxlzip.workbook.web import WebPublishing, WebPublishObjectList
...
class WorkbookPackage(Serialisable):
    ...
    def __init__(self,
        ...
        self.elem_order = {}
        for tag in WorkbookPackage.__elements__:
            self.elem_order[len(self.elem_order)] = "{%s}%s" % (SHEET_MAIN_NS, tag)
        self.elem_order[len(self.elem_order)] = "{%s}extLst" % SHEET_MAIN_NS
        self.elem_order[len(self.elem_order)] = "{%s}Ignorable" % SHEET_MAIN_NS


    def to_tree(self, elem_type=None):
        root_tag = "{http://schemas.openxmlformats.org/spreadsheetml/2006/main}workbook"
        root_type = ROOT_ELEMS[root_tag]
        tree = super(WorkbookPackage, self).to_tree(elem_type=root_type)
