...
from openpyxlzip.xml.functions import (iterparse, fromstring)

# package imports
from openpyxlzip.cell import Cell, MergedCell
from openpyxlzip.cell.text import Text
from openpyxlzip.worksheet.dimensions import (
    ColumnDimension,
    RowDimension,
...
from openpyxlzip.formatting.formatting import ConditionalFormatting
from openpyxlzip.formula.translate import Translator
from openpyxlzip.utils import (
    get_column_letter,
    coordinate_to_tuple,
    )
from openpyxlzip.utils.datetime import from_excel, from_ISO8601, WINDOWS_EPOCH
...
ALL_CHILD_IGNORE_TAGS = set([MERGE_TAG, CF_TAG, HYPERLINK_TAG, TABLE_TAG, PRINT_TAG,
                PAGE_TAG, HEADER_TAG, FILTER_TAG, VALIDATION_TAG, PROPERTIES_TAG,
                VIEWS_TAG, FORMAT_TAG, ROW_BREAK_TAG, COL_BREAK_TAG, SCENARIOS_TAG,
                DATA_TAG, DIMENSION_TAG, COLS_TAG, MARGINS_TAG,
                DRAWING_TAG])

ALL_CHILD_TAGS = set([MERGE_TAG, CF_TAG, PROT_TAG, HYPERLINK_TAG, TABLE_TAG, PRINT_TAG,
                PAGE_TAG, HEADER_TAG, FILTER_TAG, VALIDATION_TAG, PROPERTIES_TAG,
                VIEWS_TAG, FORMAT_TAG, ROW_BREAK_TAG, COL_BREAK_TAG, SCENARIOS_TAG,
                DATA_TAG, DIMENSION_TAG, CUSTOM_VIEWS_TAG, COLS_TAG, MARGINS_TAG,
                SHEET_CALC_PR_TAG, PROT_RANGES_TAG, SORT_STATE_TAG, CONSOLIDATE_TAG,
                PHONETIC_TAG, CUSTOM_PR_TAG, CELL_WATCH_TAG, IGNORED_ERRORS_TAG, SMART_TAGS_TAG,
                DRAWING_TAG, DRAWING_HF_TAG, PICTURE_TAG, OLE_OBJECTS_TAG, CONTROLS_TAG,
...
class WorkSheetParser(object):
    ...
    def parse(self):
        ...
        it = iterparse(self.source, events=["start", "end"])

        depth = 0
        for event, element in it:
            if depth == 0:
                if hasattr(element, "nsmap"):
                    self.nsmaps = element.nsmap
                attribs = dict(element.attrib)
                for key in attribs:
                    self.extra_attrib[key] = attribs[key]
            if event == 'start':
                depth += 1
                continue
            elif event == 'end':
            ...
            if tag_name in dispatcher:
                ...
                if depth == 1:
                    if tag_name in ALL_CHILD_IGNORE_TAGS:
                        continue
                    self.extra_elem[tag_name] = element
                pass
        # print(self.extra_elem.keys())
    ...
    # MattJ this just throws away extensions, keeping for test cases
    ...
    # MattJ this didn't do anything apparently, keeping for test cases
...
class WorksheetReader(object):
    ...
    def bind_merged_cells(self):
        from openpyxlzip.worksheet.cell_range import MultiCellRange
        from openpyxlzip.worksheet.merge import MergedCellRange
    ...
    def bind_properties(self):
        for k in ('print_options', 'page_margins', 'page_setup',
                  ...
                  'protection', 'extra_elem', 'nsmaps', 'extra_attrib',
                  ):
