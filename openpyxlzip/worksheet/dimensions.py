...
from openpyxlzip.utils.units import DEFAULT_COLUMN_WIDTH
...
from openpyxlzip.styles.styleable import StyleableObject
from openpyxlzip.utils.bound_dictionary import BoundDictionary
from openpyxlzip.xml.functions import Element
from openpyxlzip.xml.constants import X14AC_NS
...
class RowDimension(Dimension):
    ...
    __fields__ = Dimension.__fields__ + ('spans', 'ht', 'customFormat', 'customHeight', 's',
                                         'thickBot', 'thickTop', 'dyDescent')
    r = Alias('index')
    s = Alias('style_id')
    ht = Float(allow_none=True)
    dyDescent = String(namespace=X14AC_NS, allow_none=True)
    height = Alias('ht')
    thickBot = Bool()
    thickTop = Bool()
