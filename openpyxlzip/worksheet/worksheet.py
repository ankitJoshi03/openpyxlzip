...
from openpyxlzip.compat import (
    deprecated,
)

# package imports
from openpyxlzip.utils import (
...
from openpyxlzip.cell import Cell, MergedCell
from openpyxlzip.formatting.formatting import ConditionalFormattingList
from openpyxlzip.packaging.relationship import RelationshipList, Relationship
from openpyxlzip.workbook.child import _WorkbookChild
from openpyxlzip.workbook.defined_name import COL_RANGE_RE, ROW_RANGE_RE
from openpyxlzip.formula.translate import Translator
...
# from .ole import OleObject, ObjectPr, ObjectAnchor, OleObjects, VML_OLE_DOC_FORMAT, VML_OLE_SHAPE_FORMAT
# from openpyxlzip.drawing.spreadsheet_drawing import AnchorMarker, SpreadsheetDrawing, AnchorClientData, TwoCellAnchor
# from openpyxlzip.drawing.connector import ShapeMeta, Shape
# from openpyxlzip.drawing.properties import NonVisualDrawingProps, NonVisualDrawingShapeProps
# from openpyxlzip.descriptors.office_art import HiddenEffects, CompatExt, OfficeArtExtension, CreationId, OfficeArtExtensionList
# from openpyxlzip.chart.shapes import GraphicalProperties
# from openpyxlzip.drawing.colors import ColorChoiceDescriptor, SRGBClr
# # from openpyxlzip.drawing.fill import *
# # from openpyxlzip.packaging.relationship import Relationship, RelationshipList
# from openpyxlzip.drawing.line import LineProperties, LineEndProperties
# from openpyxlzip.drawing.effect import EffectList, OuterShadow
# # from openpyxlzip.embedding.ole_utils import ZipOLEFile, PDFOLEFile
...
from openpyxlzip.xml.constants import REL_NS, SHEET_DRAWING_NS, X14_NS, MC_NS, X14AC_NS, XR_NS, XR2_NS, XR3_NS
...
class Worksheet(_WorkbookChild):
    ...
    use :func:`openpyxlzip.workbook.Workbook.create_sheet` instead
    ...
    def add_attachments(self, attachments):
        ...
        from openpyxlzip.chart.shapes import GraphicalProperties
        from openpyxlzip.drawing.colors import ColorChoiceDescriptor, SRGBClr
        from openpyxlzip.drawing.fill import ColorChoice
        # from openpyxlzip.packaging.relationship import Relationship, RelationshipList
        from openpyxlzip.drawing.line import LineProperties, LineEndProperties
        from openpyxlzip.drawing.effect import EffectList, OuterShadow
        # from openpyxlzip.embedding.ole_utils import ZipOLEFile, PDFOLEFile
        ...
        all_anchors = []
        vml_shape_strs = []
        vml_relationship_list = RelationshipList()
        for idx, attachment in enumerate(attachments):
            attachment["ole_file"].write(attachment["ole_outfile"])
            ole_id = str(1025 + idx)
            ole_name = "Object {}".format(idx + 1)
            ole_spid = "_x0000_s{}".format(ole_id)
            ole_creationId = "{00000000-0008-0000-0000-000001040000}"
            rel = Relationship(Id="rId{0}".format(idx + 1), type="image", Target="../media/image{}.emf".format(idx + 1))
            vml_relationship_list.append(rel)
            vml_shape_str = VML_OLE_SHAPE_FORMAT.format(shape_id=ole_spid, 
                            margin_left=attachment["vml"]["margin_left"],
                            margin_top=attachment["vml"]["margin_top"],
                            width=attachment["vml"]["width"],
                            height=attachment["vml"]["height"],
                            z_index=idx+1,
                            relid="rId{0}".format(idx + 1),
                            anchor=attachment["vml"]["anchor"],
                            )
            from_anchor = AnchorMarker(col=attachment["from"]["col"],
                            colOff=attachment["from"]["colOff"],
                            row=attachment["from"]["row"],
                            rowOff=attachment["from"]["rowOff"])
            to_anchor = AnchorMarker(col=attachment["to"]["col"],
                            colOff=attachment["to"]["colOff"],
                            row=attachment["to"]["row"],
                            rowOff=attachment["to"]["rowOff"])
            anchor = ObjectAnchor(_from=from_anchor,
                            to=to_anchor,
                            moveWithCells=True,
                            sizeWithCells=None,
                            z_order=None,)
            # anchor.namespace = 
            objectPr = ObjectPr(anchor=anchor,
                            locked=None,
                            defaultSize=False,
                            _print=None,
                            disabled=None,
                            ...
                            altText=None,
                            dde=None,
                            path="xl/media/image{}.emf".format(idx + 1),
                            imgData=attachment["media"])
            ole = OleObject(objectPr=objectPr,
                            progId=attachment["progId"],
                            dvAspect=None,
                            link=None,
                            oleUpdate=None,
                            autoLoad=None,
                            shapeId=ole_id,
                            path="xl/embeddings/oleObject{}.bin".format(idx+1),
                            oleObj=attachment["ole_outfile"])
            ole_objects.append(ole)
            ...
            to_anchor = AnchorMarker(col=attachment["to"]["col"],
                            ...
                            row=attachment["to"]["row"],
                            rowOff=attachment["to"]["rowOff"])
            compatExt = CompatExt(spid=ole_spid)
            ext1 = OfficeArtExtension(uri="{63B3BB69-23CF-44E3-9099-C40C66FF867C}",
                        compatExt=compatExt)
            creationId = CreationId(id=ole_creationId)
            ext2 =  OfficeArtExtension(uri="{FF2B5EF4-FFF2-40B4-BE49-F238E27FC236}",
                        creationId=creationId)
            extLst = OfficeArtExtensionList(ext=(ext1, ext2))
            cNvPr = NonVisualDrawingProps(id=ole_id,
                        name=ole_name,
                        descr=None,
                        hidden="1",
                        title=None,
            ...
            off = Point2D(x=0, y=0)
            ext = PositiveSize2D(cx=0, cy=0)
            xfrm = Transform2D(rot=None,
                        flipH=None,
                        flipV=None,
                        off=off,
                        ext=ext,
                        chOff=None,
                        chExt=None,)
            prstGeom = PresetGeometry2D(prst="rect", avLst=GeomGuideList())
            srgbClr = SRGBClr(val="FFFFFF",
                        Ignorable="a14",
                        legacySpreadsheetColorIndex="65")
            solidFill1 = ColorChoice(srgbClr=srgbClr) #ColorChoiceDescriptor()
            srgbClr = SRGBClr(val="000000",
                        Ignorable="a14",
                        legacySpreadsheetColorIndex="64")
            solidFill2 = ColorChoice(srgbClr=srgbClr) #ColorChoiceDescriptor()
            if attachment["drawing_tail_end"]:
                tailEnd = LineEndProperties(type="none",
                            w="med",
                            len="med")
            else:
                tailEnd = None
            if attachment["drawing_prst_dash"]:
                prstDash = "solid"
            else:
                prstDash = None
            ln = LineProperties(w="9525",
                        cap=None,
                        cmpd=None,
                        algn=None,
                        noFill=None,
                        solidFill=solidFill2,
                        gradFill=None,
                        pattFill=None,
                        prstDash=prstDash,
                        custDash=None,
                        ...
                        tailEnd=tailEnd,
                        extLst=None,)
            srgbClr = SRGBClr(val="808080")
            outerShdw = OuterShadow(blurRad=None,
                        dist="35921",
                        dir="2700000",
                        srgbClr=srgbClr,
                        sx=None,
                        ...
                        algn="ctr",
                        rotWithShape="0",)
            effectLst2 = EffectList(blur=None,
                        fillOverlay=None,
                        glow=None,
                        innerShdw=None,
                        outerShdw=outerShdw,
                        ...
                        reflection=None,
                        softEdge=None,)
            # effectLst2.namespace = DRAWING_14_NS
            hiddenEffects = HiddenEffects(effectLst=effectLst2)
            ext1 = OfficeArtExtension(uri="{AF507438-7753-43E0-B8FC-AC1667EBCBE1}",
                        hiddenEffects=hiddenEffects)
            if attachment["drawing_ext_list"]:
                extLst = OfficeArtExtensionList(ext=(ext1,))
            else:
                extLst = None
            if attachment["drawing_effect_list"]:
                effectLst1 = EffectList()
            else:
                effectLst1 = None
            spPr = GraphicalProperties(bwMode="auto",
                        xfrm=xfrm,
                        noFill=None,
                        solidFill=solidFill1,
                        gradFill=None,
                        pattFill=None,
                        ln=ln,
                        effectLst=effectLst1,
                        scene3d=None,
                        custGeom=None,
                        prstGeom=prstGeom,
                        sp3d=None,
            ...
            self._legacy_images["xl/media/image{}.emf".format(idx + 1)] = attachment["media"]
            vml_shape_strs.append(VML_OLE_SHAPE_FORMAT.format(shape_id=ole_spid, 
                margin_left=attachment["vml"]["margin_left"], 
                margin_top=attachment["vml"]["margin_top"], 
                width=attachment["vml"]["width"], 
                height=attachment["vml"]["height"], 
        ...
        self.ole_objects = OleObjects(oleObject=tuple(ole_objects))

        drawing = SpreadsheetDrawing(
            twoCellAnchor=tuple(all_anchors),
            wrap_alternate_content=True,)
        self.drawings["drawing1"] = drawing
        ...
        self.extra_attrib.update({
            "{{{}}}Ignorable".format(self.nsmaps["mc"]): "x14ac xr xr2 xr3",
            "{{{}}}uid".format(self.nsmaps["xr"]): "{00000000-0001-0000-0000-000000000000}"
        })
        
        self.legacy_drawing = 'xl/drawings/vmlDrawing{0}.vml'.format(1) #TODO this will break existing
        self.vml = VML_OLE_DOC_FORMAT.format(shapes="".join(vml_shape_strs))
        self.vml_rels = vml_relationship_list
